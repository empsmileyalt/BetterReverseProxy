...
export const config = {
  runtime: 'edge', // Forces high-speed edge computing execution
};
...
export default async function handler(request) {
  ...
  if (request.headers.get('upgrade') !== 'websocket') {
    return new Response(
      ...
      "To correctly join this server, load the latest EaglercraftX 1.12 client, click the 'Direct Connect' " +
      "button on the 'Multiplayer' screen, and enter wss://better-reverse-proxy.vercel.app/ as the server address.",
      { status: 400, headers: { 'Content-Type': 'text/plain' } }
    );
  }

  // 2. Open a secure WebSocket connection pair instantly
  ...
  // 3. Pipe client inputs straight down to Velocity
  ...
  // 4. Return the active connection parameters cleanly to the browser
